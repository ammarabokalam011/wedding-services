...
<main>
    ...
    <section id="request">
        <?php
        foreach ($req as $item){
            echo $item->reservetion_time.$item->approvment_message;
            foreach ($item->services as $value){
                echo $value->service_type.$value->res_detailes.$value->service_price;
            }
            echo '<form class="singleAmountForm" method="post">
<input type="text" class="form-control" name="approvment_message">
<input type="hidden" class="form-control" value="'.$item->reservetion_id.'" name="reserve_id">
<input type="hidden" class="form-control" value="'.$id.'" name="hall_id">
<input type="submit" id="singleAmountForm" class="form-control" value="accept" formaction="'.base_url('Provider/hall_approve_accept').'">
<input type="submit" onsubmit="return show_alertDenied()" class="form-control" value="denied" formaction="'.base_url('Provider/hall_approve_denied').'">';
