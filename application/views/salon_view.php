...
<section  class="row" style="margin-left:10%; margin-top: 10%; margin-right:10% " >
    ...
    foreach ($salons as $item) {
        ...
        echo ' <div class="col-lg-6 col-md-6  content-item content-item-1 background" style="width:600px; height: 550px;"  >  
                           <h2  class="heading-agileinfo ">'.$item->salon_name.
            '<span><h3><span class="light-blue-text">Location:</span>' .$item->salon_location.
            '<span class="light-blue-text">Mobile:</span>' .$item->salon_mobile.
            '<span class="light-blue-text">Phone:</span>' .$item->salon_phone.

            '<span class="light-blue-text">Email:</span>'.$item->salon_email.'</h3>';
                   ...
                   <li  class='span4 gallery-item' data-id='id-1' data-type='illustration'>
                        <span class='gallery-hover-3col hidden-phone hidden-tablet''>
                            <span class='gallery-icons'>
                                <a href='$item->photo_salon_link' class='item-zoom-link lightbox' title='Custom Illustration' data-rel='prettyPhoto'></a>
                                <a href='gallery-single.htm' class='item-details-link'></a>
                            </span>
                        </span>
                        <a href='$item->photo_salon_link'><img width='300' height='250' src='$item->photo_salon_link' alt='Gallery'></a>
                        <span class='project-details'><a href='$item->photo_salon_link'></a>elegant wedding</span>
                    </li>";
echo '</ul></div>';
...
<div class="row" >
    <div class="col-lg-12 col-md-6 " >
        <div style="width: 650px; background-color: rgb(236,227,236); border: solid; margin: auto; padding-left: 40px; padding-top: 30px; padding-bottom: 30px; padding-right: 10px">
            ...
            echo ' <form action="'.base_url('salons/reserve').'" method="post">';
            echo '<input type="hidden" name="hall_id" value="' .$id.'" >
             <h4 style="padding-left: 23%" class="white-w3ls">Reserve Your Salon</h4>
            ...
            foreach ($services as $service){
                echo '<input type="hidden" name="salon_id" value="' .$id.'" >';
                echo '<label class="ainer">'.$service->service_type.' &nbsp;&nbsp;<h4 style="color: red">'.$service->service_price.' LS</h4>';
                echo '<input class="service" price="'.$service->service_price.'" onclick="calc()" value="'.$service->service_id.'" type="checkbox" name="service[]">
...
<script>
    function calc(){
        ...
        for(var i=0;i<arr.length;i++){
            if(arr[i].checked)
                tot += parseInt(arr[i].getAttribute('price'));
        }
