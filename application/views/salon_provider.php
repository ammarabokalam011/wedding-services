...
<main>
    ...
    <section id="request">
        ...
        foreach ($req as $item){
            $total=0;
            echo $item->reserve_time;
            echo $item->approvment_message;
            foreach ($item->services as $service){
                echo $service->service_type;
                echo $service->res_detailes;
...
<input type="hidden" class="form-control" value="'.$item->reser_id.'" name="reserve_id">
<input type="hidden" class="form-control" value="'.$id.'" name="salon_id">
<input type="submit" id="singleAmountForm" class="form-control" value="accept" formaction="'.base_url('Provider/salon_approve_accept').'">
<input type="submit" onsubmit="return show_alertDenied()" class="form-control" value="denied" formaction="'.base_url('Provider/salon_approve_denied').'">';
            echo '
</form><br>';
